...
// GetEstadoCumplidoById retrieves EstadoCumplido by Id. Returns error if
// Id doesn't exist
...
func GetAllEstadoCumplido(query map[string]string, fields []string, sortby []string, order []string,
	...
	o := orm.NewOrm()
	qs := o.QueryTable(new(EstadoCumplido))
	// query k=v
	for k, v := range query {
...
// UpdateEstadoCumplido updates EstadoCumplido by Id and returns error if
...
func DeleteEstadoCumplido(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&EstadoCumplido{Id: id}); err == nil {
